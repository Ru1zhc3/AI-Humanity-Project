...
- Reads `Building Dataset/final_statements.csv`
...
- Adds:
  ...
  - `error_message`
- Writes:
  - `Evaluation/codex_outputs/codex_eval_results.csv`
  - `Evaluation/codex_outputs/codex_eval_review.csv`
  - `Evaluation/codex_outputs/codex_eval_checkpoint.json`
...
```powershell
python Evaluation\run_codex_evaluation.py --max-items 10
```

Full run:

```powershell
python Evaluation\run_codex_evaluation.py
```

Useful flags:

```powershell
python Evaluation\run_codex_evaluation.py --model gpt-5-mini --batch-size 15
python Evaluation\run_codex_evaluation.py --overwrite
python Evaluation\run_codex_evaluation.py --no-retry-errors
python Evaluation\run_codex_evaluation.py --input "E:\path\to\other.csv"
```
...
```powershell
C:\Users\Yi Ping\.cache\codex-runtimes\codex-primary-runtime\dependencies\python\python.exe Evaluation\run_codex_workbook_evaluation.py --model gpt-5.4-mini --batch-size 100
```

This runner reads:

- `Building Dataset/political_statements_perturbed_v2.xlsx`
- all `.xlsx` files in `Building Dataset/Perturbed Statement Translations/`
