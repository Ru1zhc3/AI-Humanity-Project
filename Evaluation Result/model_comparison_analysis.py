...
RESULT_DIR = ROOT / "Evaluation Result"
...
def load_codex_final() -> pd.DataFrame:
    path = RESULT_DIR / "codex_final_multilingual_review.csv"
...
def load_codex_perturbation() -> pd.DataFrame:
    path = RESULT_DIR / "codex_workbook_eval_results.csv"
    df = pd.read_csv(path, encoding="utf-8")
    out = pd.DataFrame(
        {
            "model": "Codex gpt-5.4-mini",
...
def load_gemma_perturbation() -> pd.DataFrame:
    folder = RESULT_DIR / "gemma4perturbation"
    frames: list[pd.DataFrame] = []
...
def main() -> None:
    ...
    proposal = docx_text(ROOT / "Team Cyan AI and Humanity Project Idea.docx")
    proposal_hits = []
